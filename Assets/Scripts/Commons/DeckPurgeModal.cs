...
/// <summary>
/// ユニークスキル「カード除去」（塔覇の本(A)装備中）の選択モーダル（コード生成・羊皮紙スタイル）。
/// バフカード山札QRを読んだとき、抽選の前に山札から取り除くカードを1種類選ばせる。
/// カードをタップ＝そのカードを山札から取り除いて抽選へ、
/// 「取りのぞかずに引く」＝除去せずそのまま抽選へ進む。
/// 暗幕タップでは閉じない（誤タップでスキルが流れるのを防ぐ）。
/// </summary>
public static class DeckPurgeModal
{
    ...
    /// <summary>
    /// 山札の内容（種類→枚数）を一覧表示し、取り除くカードの決定を onDecided で返す。
    /// 取り除かない場合は null を渡す。onDecided は必ず1回だけ呼ばれる。
        ...
        string notice = null)
    {
        ...
        if (canvas == null)
        {
            Debug.LogWarning("[DeckPurgeModal] Canvasが見つからないため除去なしで進めます");
            onDecided(null);
            return;
        }
        ...
        var dim = new GameObject("__DeckPurgeModal");
        ...
        void Decide(BuffCardType? removed)
        {
            if (decided) return;
            decided = true;
